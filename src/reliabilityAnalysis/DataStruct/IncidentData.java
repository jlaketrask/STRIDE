...
public class IncidentData {
    ...
    public IncidentData(Seed seed) {
        this.seed = seed;
        incidentProbabilities = new float[12][6];
        
        // Reading in data from seed (if data exists)
        incidentFreqMonth = new float[12];
        //incidentFreqMonth = seed.getIncidentFrequency();
        setIncidentFrequency(seed.getIncidentFrequency());
        
        incidentDistribution = new float[5];
        if (seed.getIncidentDuration()==null) {
            incidentDurationInfo = new float[5][4];
        } else {
            incidentDurationInfo=seed.getIncidentDuration();
        }
        ...
        if (seed.getIncidentSAF()==null) {
            ...
            incidentFFSAFs = seed.getIncidentSAF();
        }
        if (seed.getIncidentCAF()==null) {
            ...
            incidentCAFs = seed.getIncidentCAF();
        }
        if (seed.getIncidentDAF()==null) {
            ...
            incidentDAFs = seed.getIncidentDAF();
        }
        if (seed.getIncidentLAF()==null) {
            ...
            incidentLAFs = seed.getIncidentLAF();
        }
        
        if (seed.getIncidentCrashRatio()==0.0f) {
            crashRateRatio = 4.9f; // National default
        } else {
            crashRateRatio = seed.getIncidentCrashRatio();
        }
    ...
    public void useFrequenciesFromSeed() {
        if (this.seed!=null) {
            float[] tempFreqArr = seed.getIncidentFrequency();
            if (tempFreqArr!=null) {
                for (int month = 0; month < tempFreqArr.length; month++) {
                    this.incidentFreqMonth[month]=seed.getIncidentFrequency()[month];
