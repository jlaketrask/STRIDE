    ...
    private void calculateDrawSize() {
        ...
        if (seed != null) {
            ...
            for (int seg = 0; seg < seed.getValueInt(CEConst.IDS_NUM_SEGMENT); seg++) {
                maxNumOfLanes = Math.max(seed.getValueInt(CEConst.IDS_MAIN_NUM_LANES_IN_AND_ATDM, seg, period, scen, atdm), maxNumOfLanes);
            }
    ...
    private void drawFreeway(Graphics2D g) {
        ...
        for (int seg = 0; seg < seed.getValueInt(CEConst.IDS_NUM_SEGMENT); seg++) {
            ...
            if (maxNumOfMLLanes > 0) {
                ...
                if (!isShowInput) {
                    drawMLOutput(g, currX, currY + (maxNumOfMLLanes - seed.getValueInt(CEConst.IDS_ML_NUM_LANES, seg)) * heightPerLane, seg);
                }

                //draw separation
                if (seed.getValueInt(CEConst.IDS_ML_NUM_LANES, seg) > 0) {
                    g.setColor(findColor("GPMLSP"));
                    switch (seed.getValueInt(CEConst.IDS_ML_SEPARATION_TYPE, seg)) {
                        case CEConst.ML_SEPARATION_BARRIER:
                            //g.setStroke(SEPARATION_BARRIER_SOLID);
//                            g.drawLine(currX,
//                                    currY + maxNumOfMLLanes * heightPerLane + GPMLSpace / 2,
//                                    currX + (int) (seed.getValueInt(CEConst.IDS_ML_SEGMENT_LENGTH_FT, seg) / 5280.0 * widthPerMile),
//                                    currY + maxNumOfMLLanes * heightPerLane + GPMLSpace / 2);
                            g.fillRect(currX,
...
//                            g.setStroke(SEPARATION_BUFFER_DASH);
//                            g.drawLine(currX,
//                                    currY + maxNumOfMLLanes * heightPerLane + GPMLSpace / 2,
//                                    currX + (int) (seed.getValueInt(CEConst.IDS_ML_SEGMENT_LENGTH_FT, seg) / 5280.0 * widthPerMile),
//                                    currY + maxNumOfMLLanes * heightPerLane + GPMLSpace / 2);
                ...
                switch (seed.getValueInt(CEConst.IDS_ML_SEGMENT_TYPE, seg)) {
                    case CEConst.SEG_TYPE_B:
                        drawMLBasicSegment(g, currX, currY + (maxNumOfMLLanes - seed.getValueInt(CEConst.IDS_ML_NUM_LANES, seg)) * heightPerLane, seg);
                        break;
                    case CEConst.SEG_TYPE_R:
                        drawMLBasicSegment(g, currX, currY + (maxNumOfMLLanes - seed.getValueInt(CEConst.IDS_ML_NUM_LANES, seg)) * heightPerLane, seg);
                        break;
                    case CEConst.SEG_TYPE_ONR:
                        drawMLOnRampSegment(g, currX, currY + (maxNumOfMLLanes - seed.getValueInt(CEConst.IDS_ML_NUM_LANES, seg)) * heightPerLane, seg);
                        break;
                    case CEConst.SEG_TYPE_OFR:
                        drawMLOffRampSegment(g, currX, currY + (maxNumOfMLLanes - seed.getValueInt(CEConst.IDS_ML_NUM_LANES, seg)) * heightPerLane, seg);
                        break;
                    case CEConst.SEG_TYPE_W:
                        drawMLWeavingSegment(g, currX, currY + (maxNumOfMLLanes - seed.getValueInt(CEConst.IDS_ML_NUM_LANES, seg)) * heightPerLane, seg);
                        break;
                    case CEConst.SEG_TYPE_ACS:
                        drawMLAccessSegment(g, currX, currY + (maxNumOfMLLanes - seed.getValueInt(CEConst.IDS_ML_NUM_LANES, seg)) * heightPerLane, seg);
                        break;
                }
            ...
            if (seed.getValueInt(CEConst.IDS_RL_LAFWZ, seg, period, scen, atdm) < 0) {
                fillWorkZone(g,
                        currX + 1,
                        currY,
                        (int) (seed.getValueInt(CEConst.IDS_SEGMENT_LENGTH_FT, seg) / 5280.0 * widthPerMile) - 1,
                        -heightPerLane * seed.getValueInt(CEConst.IDS_RL_LAFWZ, seg, period, scen, atdm));
            }

            //dawy incident lane closure
            g.setColor(findColor("INC"));
            if (seed.getValueInt(CEConst.IDS_RL_LAFI, seg, period, scen, atdm) < 0) {
                g.fillRect(currX + 1,
                        currY - heightPerLane * seed.getValueInt(CEConst.IDS_RL_LAFWZ, seg, period, scen, atdm),
                        (int) (seed.getValueInt(CEConst.IDS_SEGMENT_LENGTH_FT, seg) / 5280.0 * widthPerMile) - 1,
                        -heightPerLane * seed.getValueInt(CEConst.IDS_RL_LAFI, seg, period, scen, atdm));
            }
            ...
            //draw segment index
            g.setColor(findColor("TEXT"));
            g.drawString(Integer.toString(seg + 1), currX + 1, (getSize().height - drawHeight) / 2 + verEdge - 7);
        ...
        if (maxNumOfMLLanes > 0) {
            ...
            for (int lane = maxNumOfMLLanes; lane >= 1; lane--) {
                ...
                for (int seg = 0; seg < seed.getValueInt(CEConst.IDS_NUM_SEGMENT); seg++) {
                    ...
                    if (seed.getValueInt(CEConst.IDS_ML_NUM_LANES, seg) >= lane) {
                        if (start == -1) {
                            start = seg;
                            startX = currX;
                        }
                    ...
                    if (start > -1 && (seed.getValueInt(CEConst.IDS_ML_NUM_LANES, seg) < lane || seg == seed.getValueInt(CEConst.IDS_NUM_SEGMENT) - 1)) {
                        drawDiamonds(g,
                                startX,
                                currY,
    ...
    private void drawArrows(Graphics2D g, int x, int y) {
        ...
        for (int i = 0; i < seed.getValueInt(CEConst.IDS_MAIN_NUM_LANES_IN_AND_ATDM, 0); i++) {
            drawArrow(g, x, currY);
            currY += heightPerLane;
        }
    }
    ...
    private void drawBasicSegment(Graphics2D g, int currX, int currY, int seg) {
        ...
        if (seg < seed.getValueInt(CEConst.IDS_NUM_SEGMENT) - 1) {
            g.drawLine(x + (int) (seed.getValueInt(CEConst.IDS_SEGMENT_LENGTH_FT, seg) / 5280.0 * widthPerMile),
                    ...
                    y + heightPerLane * Math.max(seed.getValueInt(CEConst.IDS_MAIN_NUM_LANES_IN_AND_ATDM, seg, period, scen, atdm), seed.getValueInt(CEConst.IDS_MAIN_NUM_LANES_IN_AND_ATDM, seg + 1, period, scen, atdm)) + separationExtention);
        }

        g.setStroke(LANE_SOLID);
        //mainline
        for (int laneIndex = 0; laneIndex <= seed.getValueInt(CEConst.IDS_MAIN_NUM_LANES_IN_AND_ATDM, seg, period, scen, atdm); laneIndex++) {
    ...
    private void drawAccessSegment(Graphics2D g, int currX, int currY, int seg) {
        ...
        //mainline
        g.setStroke(LANE_SOLID);
        for (int laneIndex = 1; laneIndex <= seed.getValueInt(CEConst.IDS_MAIN_NUM_LANES_IN_AND_ATDM, seg, period, scen, atdm); laneIndex++) {
            g.drawLine(x, y, x + (int) (seed.getValueInt(CEConst.IDS_SEGMENT_LENGTH_FT, seg) / 5280.0 * widthPerMile), y);
            y += heightPerLane;
        }
    }
    ...
    private void drawOnRampSegment(Graphics2D g, int currX, int currY, int seg) {
        ...
        if (seg < seed.getValueInt(CEConst.IDS_NUM_SEGMENT) - 1) {
            g.drawLine(x + (int) (seed.getValueInt(CEConst.IDS_SEGMENT_LENGTH_FT, seg) / 5280.0 * widthPerMile),
                    y - separationExtention,
                    x + (int) (seed.getValueInt(CEConst.IDS_SEGMENT_LENGTH_FT, seg) / 5280.0 * widthPerMile),
                    y + heightPerLane * Math.max(seed.getValueInt(CEConst.IDS_MAIN_NUM_LANES_IN_AND_ATDM, seg, period, scen, atdm),
                            seed.getValueInt(CEConst.IDS_MAIN_NUM_LANES_IN_AND_ATDM, seg + 1, period, scen, atdm)) + separationExtention);
        }
    ...
    private void drawMLOutput(Graphics2D g, int currX, int currY, int seg) {
        if (scen < 0 || scen > seed.getValueInt(CEConst.IDS_NUM_SCEN)) {
            ...
            g.setColor(
                    findColor(seed.getValueString(CEConst.IDS_ML_DENSITY_BASED_LOS, seg, period, scen, atdm).toUpperCase().substring(0, 1)));
