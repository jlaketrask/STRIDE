...
public class Seed implements Serializable {
     ...
     */
    private CETime inStartTime = new CETime(0, 0);
    /**
     * End time of analysis period
     */
    private CETime inEndTime = new CETime(24, 0);
    /**
    ...
    /**
     * Reliability scenarios, NOT include default/empty seed scenario
     */
    private Scenario RL_Scenarios;
    /**
     * Seed file date
     */
    ...
    /**
     * Specified demand multiplier
     */
    private float[][] RL_SpecifiedDemand = new float[12][7];
    /**
     ...
     * Incident frequency
     */
    private float[] RL_IncidentFrequency;
    /**
     * Incident duration
     */
    private float[][] RL_IncidentDuration;
    /**
     * Incident CAF
     */
    private float[][] RL_IncidentCAF;
    /**
     * Incident DAF
     */
    private float[][] RL_IncidentDAF;
    /**
     * Incident SAF
     */
    private float[][] RL_IncidentSAF;
    /**
     * Incident LAF
     */
    private int[][] RL_IncidentLAF;
    /**
     * Incident over crash ratio
     */
    private float RL_IncidentCrashRatio = 4.9f;
    // </editor-fold>
     ...
     * @return
     */
    private float getRLOAF(int scen, int seg, int period) {
        if (scen == 0) {
            return 1;
        } else {
            return RL_Scenarios.OAF().get(scen - 1, seg, period);
        }
    }
     ...
     * @return
     */
    private float getRLDAF(int scen, int seg, int period) {
        if (scen == 0) {
            return 1;
        } else {
            return RL_Scenarios.DAF().get(scen - 1, seg, period);
        }
    }
     ...
     * @return
     */
    private float getRLSAF(int scen, int seg, int period) {
        if (scen == 0) {
            return 1;
        } else {
            return RL_Scenarios.SAF().get(scen - 1, seg, period);
        }
    }
     ...
     * @return
     */
    public float getRLCAF(int scen, int seg, int period) {
        if (scen == 0) {
            return 1;
        } else {
            return RL_Scenarios.CAF().get(scen - 1, seg, period);
        }
    }
     ...
     * @return
     */
    private int getRLLAFI(int scen, int seg, int period) {
        if (scen == 0) {
            return 0;
        } else {
            return RL_Scenarios.LAFI().get(scen - 1, seg, period);
        }
    }
     ...
     * @return
     */
    private int getRLLAFWZ(int scen, int seg, int period) {
        if (scen == 0) {
            return 0;
        } else {
            return RL_Scenarios.LAFWZ().get(scen - 1, seg, period);
        }
    }
     ...
     * @return
     */
    float getRLAndATDMOAF(int scen, int atdm, int seg, int period) {
        return getRLOAF(scen, seg, period) * getATDMOAF(scen, atdm, seg, period);
    }
     ...
     * @return
     */
    float getRLAndATDMDAF(int scen, int atdm, int seg, int period) {
        return getRLDAF(scen, seg, period) * getATDMDAF(scen, atdm, seg, period);
    }
     ...
     * @return
     */
    float getRLAndATDMSAF(int scen, int atdm, int seg, int period) {
        return getRLSAF(scen, seg, period) * getATDMSAF(scen, atdm, seg, period);
    }
     ...
     * @return
     */
    float getRLAndATDMCAF(int scen, int atdm, int seg, int period) {
        return getRLCAF(scen, seg, period) * getATDMCAF(scen, atdm, seg, period);
    }
     ...
     * @return
     */
    int getRLAndATDMLAF(int scen, int atdm, int seg, int period) {
        return getRLLAFI(scen, seg, period) + getRLLAFWZ(scen, seg, period) + getATDMLAF(scen, atdm, seg, period);
    }
    // </editor-fold>
    ...
    public Seed() {
        resetBuffer();
        for (float[] demand : RL_SpecifiedDemand) {
    ...
    public void generateSegments(int numSegment) {
        generateSegments(numSegment, (inEndTime.toMinute() - inStartTime.toMinute()) / LENGTH_OF_EACH_PERIOD.toMinute());
    }
     ...
     *
     * @param numSegment number of GPSegments
     * @param numPeriod number of analysis periods
     */
    public void generateSegments(int numSegment, int numPeriod) {
        this.inNumPeriod = numPeriod;

        GPSegments = generateSegments(numSegment, numPeriod, CEConst.SEG_TYPE_GP);

        if (inManagedLaneUsed) {
            MLSegments = generateSegments(numSegment, numPeriod, CEConst.SEG_TYPE_ML);
    ...
    public String addPeriod(int numPeriodToBeAdded, boolean isAtBeginning) {
        if (numPeriodToBeAdded > 0) {
            try {
                inNumPeriod += numPeriodToBeAdded;
                cleanScenarios();
    ...
    private void _addPeriod(GPMLSegment segment, int numPeriodToBeAdded, boolean isAtBeginning) {
        if (isAtBeginning) {
            for (int count = 0; count < numPeriodToBeAdded; count++) {
                ...
                segment.inOffDemand_veh.add(0, segment.inRRDemand_veh.get(0));
            ...
            for (int count = 0; count < numPeriodToBeAdded; count++) {
                ...
                segment.inOffDemand_veh.add(segment.inRRDemand_veh.get(0));
    ...
    public String delPeriod(int numPeriodToBeDeleted, boolean isFromBeginning) {
        if (numPeriodToBeDeleted > 0) {
            try {
                inNumPeriod -= numPeriodToBeDeleted;
                cleanScenarios();
...
//            pOutMLDensityIA_pc = CEHelper.float_2D_normal(numScen + 1, numPeriod, 0); //pc/mi/lane
    ...
    private void analyze(int scen, int atdm, int thread) {
        for (int period = 0; period < inNumPeriod; period++) {
            if (isUnderSat(thread, period)) {
                //System.out.println("Scenario: " + scen + " Period: " + period + " Under Sat");
                for (GPMLSegment segment : GPSegments) {
                ...
                //System.out.println("Scenario: " + scen + " Period: " + period + " Over Sat");
    ...
    private boolean isUnderSat(int thread, int period) {
        for (GPMLSegment segment : GPSegments) {
            ...
            if (period > 0
                    && (segment.Q[thread][period - 1] > CEConst.ZERO || segment.ONRQ[thread][period - 1] > CEConst.ZERO)) {
                return false;
            }
        }
        return true;
    ...
    private void summary(int scen, int atdm, int thread) {
        ...
        synchronized (this) {
            ...
            for (int period = 0; period < inNumPeriod; period++) {
                for (int seg = 0; seg < GPSegments.size(); seg++) {
                    if (pOutMaxVC.get(scenATDM) == null) {
                        System.out.println("Null " + scenATDM + " " + period + " " + seg + " " + thread);
                    }
        ...
        try {
            ...
            switch (ID) {
                ...
                case CEConst.IDS_NUM_PERIOD:
                    inNumPeriod = Integer.parseInt(value.toString());
...
//                        }
                    ...
                    break;
                case CEConst.IDS_RL_OAF:
                    if (scen > 0) {
                        RL_Scenarios.OAF().set(Float.parseFloat(value.toString()), scen - 1, seg, period);
                        RL_ScenarioInfo.get(scen).statusRL = CEConst.SCENARIO_INPUT_ONLY;
                        deleteAllATDM();
//                        for (HashMap<Integer, ATDMScenario> ATDMSet : ATDMSets) {
//                            if (ATDMSet.get(scen) != null) {
...
//                        }
                    ...
                    break;
                case CEConst.IDS_RL_DAF:
                    if (scen > 0) {
                        RL_Scenarios.DAF().set(Float.parseFloat(value.toString()), scen - 1, seg, period);
                        RL_ScenarioInfo.get(scen).statusRL = CEConst.SCENARIO_INPUT_ONLY;
                        deleteAllATDM();
//                        for (HashMap<Integer, ATDMScenario> ATDMSet : ATDMSets) {
//                            if (ATDMSet.get(scen) != null) {
...
//                        }
                    ...
                    break;
                case CEConst.IDS_RL_SAF:
                    if (scen > 0) {
                        RL_Scenarios.SAF().set(Float.parseFloat(value.toString()), scen - 1, seg, period);
                        RL_ScenarioInfo.get(scen).statusRL = CEConst.SCENARIO_INPUT_ONLY;
                        deleteAllATDM();
//                        for (HashMap<Integer, ATDMScenario> ATDMSet : ATDMSets) {
//                            if (ATDMSet.get(scen) != null) {
...
//                        }
                    ...
                    break;
                case CEConst.IDS_RL_LAFI:
                    if (scen > 0) {
                        RL_Scenarios.LAFI().set(-Math.abs(Integer.parseInt(value.toString())), scen - 1, seg, period);
                        RL_ScenarioInfo.get(scen).statusRL = CEConst.SCENARIO_INPUT_ONLY;
                        deleteAllATDM();
//                        for (HashMap<Integer, ATDMScenario> ATDMSet : ATDMSets) {
//                            if (ATDMSet.get(scen) != null) {
...
//                        }
                    ...
                    break;
                case CEConst.IDS_RL_LAFWZ:
                    if (scen > 0) {
                        RL_Scenarios.LAFWZ().set(-Math.abs(Integer.parseInt(value.toString())), scen - 1, seg, period);
                        RL_ScenarioInfo.get(scen).statusRL = CEConst.SCENARIO_INPUT_ONLY;
                        deleteAllATDM();
//                        for (HashMap<Integer, ATDMScenario> ATDMSet : ATDMSets) {
//                            if (ATDMSet.get(scen) != null) {
...
//                        }
                ...
                case CEConst.IDS_ATDM_CAF:
                    if (atdm >= 0) {
                        ...
                        //RL_ScenarioInfo.get(scen).getAtdmScenario().CAF().set(Float.parseFloat(value.toString()), atdm, seg, period);
                        //RL_ScenarioInfo.get(scen).statusATDM[atdm] = CEConst.SCENARIO_INPUT_ONLY;
                        resetBuffer();
                    }
                    break;
                case CEConst.IDS_ATDM_SAF:
                    if (atdm >= 0) {
                        ...
                        ATDMSets.get(atdm).get(scen).setStatus(CEConst.SCENARIO_INPUT_ONLY);
                        //RL_ScenarioInfo.get(scen).getAtdmScenario().SAF().set(Float.parseFloat(value.toString()), atdm, seg, period);
                        //RL_ScenarioInfo.get(scen).statusATDM[atdm] = CEConst.SCENARIO_INPUT_ONLY;
                        resetBuffer();
                    }
                    break;
                case CEConst.IDS_ATDM_OAF:
                    if (atdm >= 0) {
                        ...
                        ATDMSets.get(atdm).get(scen).setStatus(CEConst.SCENARIO_INPUT_ONLY);
                        //RL_ScenarioInfo.get(scen).getAtdmScenario().OAF().set(Float.parseFloat(value.toString()), atdm, seg, period);
                        //RL_ScenarioInfo.get(scen).statusATDM[atdm] = CEConst.SCENARIO_INPUT_ONLY;
                        resetBuffer();
                    }
                    break;
                case CEConst.IDS_ATDM_DAF:
                    if (atdm >= 0) {
                        ...
                        ATDMSets.get(atdm).get(scen).setStatus(CEConst.SCENARIO_INPUT_ONLY);
                        //RL_ScenarioInfo.get(scen).getAtdmScenario().DAF().set(Float.parseFloat(value.toString()), atdm, seg, period);
                        //RL_ScenarioInfo.get(scen).statusATDM[atdm] = CEConst.SCENARIO_INPUT_ONLY;
                        resetBuffer();
                    }
                    break;
                case CEConst.IDS_ATDM_LAF:
                    if (atdm >= 0) {
                        ...
                        ATDMSets.get(atdm).get(scen).setStatus(CEConst.SCENARIO_INPUT_ONLY);
                        //RL_ScenarioInfo.get(scen).getAtdmScenario().LAF().set(Integer.parseInt(value.toString()), atdm, seg, period);
                        //RL_ScenarioInfo.get(scen).statusATDM[atdm] = CEConst.SCENARIO_INPUT_ONLY;
                        resetBuffer();
                    }
                    break;
    ...
    public int getValueInt(String ID, int seg, int period, int scen, int atdm) {
        ...
        try {
            ...
            switch (ID) {
                ...
                case CEConst.IDS_RL_LAFI:
                    return getRLLAFI(scen, seg, period);
                case CEConst.IDS_RL_LAFWZ:
                    return getRLLAFWZ(scen, seg, period);
                case CEConst.IDS_ATDM_LAF:
    ...
    public float getValueFloat(String ID, int seg, int period, int scen, int atdm) {
        ...
        try {
            ...
            switch (ID) {
                ...
                case CEConst.IDS_RL_CAF:
                    return getRLCAF(scen, seg, period);
                case CEConst.IDS_RL_OAF:
                    return getRLOAF(scen, seg, period);
                case CEConst.IDS_RL_DAF:
                    return getRLDAF(scen, seg, period);
                case CEConst.IDS_RL_SAF:
                    return getRLSAF(scen, seg, period);
                case CEConst.IDS_ATDM_CAF:
...
//                case CEConst.IDS_CB_P_INFLUENCED_DENSITY_PC:
     ...
     *
     * @param scenarios generated 2-D RL scenarios [numSegments][numScenarios]
     * (Do NOT include default scenario)
     * @param scenarioInfo reliability analysis scenario information
     * @return whether setting scenarios is successful
     */
    public String setRLScenarios(Scenario scenarios, ArrayList<ScenarioInfo> scenarioInfo) {
        try {
            cleanScenarios();
            inNumScen = scenarios.size();
            this.RL_Scenarios = scenarios;
    ...
    public void setSeedFileDate(CEDate seedFileDate) {
        this.RL_SeedFileDate = seedFileDate;
    }
     ...
     */
    public void setSpecifiedDemand(float[][] specifiedDemand) {
        this.RL_SpecifiedDemand = specifiedDemand;
    }
    ...
    /**
     * Getter for incident frequency
     *
     * @return incident frequency
     */
    public float[] getIncidentFrequency() {
        return RL_IncidentFrequency;
    }

    /**
     * Setter for incident frequency
     *
     * @param incidentFrequency incident frequency
     */
    public void setIncidentFrequency(float[] incidentFrequency) {
        if (this.RL_IncidentFrequency == null) {
            this.RL_IncidentFrequency = new float[incidentFrequency.length];
        }
        for (int month = 0; month < incidentFrequency.length; month++) {
            this.RL_IncidentFrequency[month] = incidentFrequency[month];
        }
        //this.incidentFrequency = incidentFrequency;  // Old way, just sets a pointer
    }

    /**
     * Getter for incident duration
     *
     * @return incident duration
     */
    public float[][] getIncidentDuration() {
        return RL_IncidentDuration;
    }

    /**
     * Setter for incident duration
     *
     * @param incidentDuration incident duration
     */
    public void setIncidentDuration(float[][] incidentDuration) {
        this.RL_IncidentDuration = incidentDuration;
    }

    /**
     * Getter for incident CAF
     *
     * @return incident CAF
     */
    public float[][] getIncidentCAF() {
        return RL_IncidentCAF;
    }

    /**
     * Setter for incident CAF
     *
     * @param incidentCAF incident CAF
     */
    public void setIncidentCAF(float[][] incidentCAF) {
        this.RL_IncidentCAF = incidentCAF;
    }

    /**
     * Getter for incident DAF
     *
     * @return incident DAF
     */
    public float[][] getIncidentDAF() {
        return RL_IncidentDAF;
    }

    /**
     * Setter for incident DAF
     *
     * @param incidentDAF incident DAF
     */
    public void setIncidentDAF(float[][] incidentDAF) {
        this.RL_IncidentDAF = incidentDAF;
    }

    /**
     * Getter for incident SAF
     *
     * @return incident SAF
     */
    public float[][] getIncidentSAF() {
        return RL_IncidentSAF;
    }

    /**
     * Setter for incident SAF
     *
     * @param incidentSAF incident SAF
     */
    public void setIncidentSAF(float[][] incidentSAF) {
        this.RL_IncidentSAF = incidentSAF;
    }

    /**
     * Getter for incident LAF
     *
     * @return incident LAF
     */
    public int[][] getIncidentLAF() {
        return RL_IncidentLAF;
    }
     ...
     * @param incidentLAF
     */
    public void setIncidentLAF(int[][] incidentLAF) {
        this.RL_IncidentLAF = incidentLAF;
    }

    /**
     * Getter for incident crash ratio
     *
     * @return incident crash ratio
     */
    public float getIncidentCrashRatio() {
        return RL_IncidentCrashRatio;
    }
     ...
     */
    public void setIncidentCrashRatio(float incidentCrashRatio) {
        this.RL_IncidentCrashRatio = incidentCrashRatio;
    }
