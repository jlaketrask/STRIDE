...
class GPMLSegment implements Serializable {
        ...
        for (int period = 0; period < inNumPeriod; period++) {
            ...
            if (inType == CEConst.SEG_TYPE_W) {
                value = inRRDemand_veh.get(period)
                        * Math.min(inUOAF.get(period) * seed.getRLAndATDMOAF(scen, atdm, inIndex, period),
                                inUDAF.get(period) * seed.getRLAndATDMDAF(scen, atdm, inIndex, period));
            ...
            if (inType == CEConst.SEG_TYPE_ACS) {
                ...
                float minODAF = Math.min(inUOAF.get(period) * seed.getRLAndATDMOAF(scen, atdm, inIndex, period),
                        inUDAF.get(period) * seed.getRLAndATDMDAF(scen, atdm, inIndex, period));
    ...
    private void calDemand_Mainline(int scen, int atdm, int thread, int period) {
        ...
        if (inUpSeg == null) {
            ...
            float value = inMainlineDemand_veh.get(period)
                    * inUOAF.get(period)
                    * seed.getRLAndATDMOAF(scen, atdm, inIndex, period);
            ...
            float value = inType == CEConst.SEG_TYPE_ONR || inType == CEConst.SEG_TYPE_W || inType == CEConst.SEG_TYPE_ACS
                    ? inOnDemand_veh.get(period) * inUOAF.get(period) * seed.getRLAndATDMOAF(scen, atdm, inIndex, period) : 0;
            ...
            value = inType == CEConst.SEG_TYPE_OFR || inType == CEConst.SEG_TYPE_W || inType == CEConst.SEG_TYPE_ACS
                    ? inOffDemand_veh.get(period) * inUDAF.get(period) * seed.getRLAndATDMDAF(scen, atdm, inIndex, period) : 0;
    ...
    private void calFFS(int scen, int atdm, int thread) {
        for (int period = 0; period < inNumPeriod; period++) {
            scenMainlineFFS[thread][period]
                    = inMainlineFFS.get(period) * inUSAF.get(period) * seed.getRLAndATDMSAF(scen, atdm, inIndex, period);
            scenOnFFS[thread][period]
                    = inOnFFS.get(period) * inUSAF.get(period) * seed.getRLAndATDMSAF(scen, atdm, inIndex, period);
            scenOffFFS[thread][period]
                    = inOffFFS.get(period) * inUSAF.get(period) * seed.getRLAndATDMSAF(scen, atdm, inIndex, period);
        }
    }
            ...
            for (int period = 0; period < inNumPeriod; period++) {
                scenMainlineNumLanes[thread][period]
                        = Math.max(1, inMainlineNumLanes.get(period)
                                + seed.getRLAndATDMLAF(scen, atdm, inIndex, period));
            }
            for (int period = 0; period < inNumPeriod; period++) {
                if (seed.isManagedLaneUsed()) {
    ...
    private float funcBasicMainlineCapacity(int scen, int atdm, int thread, int period) {
        ...
        return result * inUCAF.get(period) * seed.getRLAndATDMCAF(scen, atdm, inIndex, period); //vph
    }
    ...
    private float funcWeaveMainlineCapacity(int scen, int atdm, int thread, int period) {
        ...
        return cW * inUCAF.get(period) * seed.getRLAndATDMCAF(scen, atdm, inIndex, period); //vph
    }
    ...
    private float funcIdealCapacity(float freeFlowSpeed, int thread, int period) {
        ...
        switch (scenType[thread][period]) {
            case CEConst.SEG_TYPE_ONR:
            case CEConst.SEG_TYPE_OFR:
                return 1800 + 20 * (Math.min(70, freeFlowSpeed) - 50);
            default:
                return 2200 + 10 * (Math.min(70, freeFlowSpeed) - 50);
            //return 2400 - Math.max(0, (70 - freeFlowSpeed)) * 10;
        }
    }
    ...
    private float funcBasicSpeed(int status, int scen, int atdm, int thread, int period) {
        ...
        if (inGPMLType == CEConst.SEG_TYPE_GP) {
            ...
            float CAF = inUCAF.get(period) * seed.getRLAndATDMCAF(scen, atdm, inIndex, period);
    ...
    private float funcWeaveSpeed(int status, int scen, int atdm, int thread, int period) {
        if (!(inUCAF.get(period) * seed.getRLAndATDMCAF(scen, atdm, inIndex, period) != 1 && status == CEConst.STATUS_BG)) {
