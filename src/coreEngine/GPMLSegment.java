...
class GPMLSegment implements Serializable {
        ...
        for (int period = 0; period < inNumPeriod; period++) {
            ...
            if (inType == CEConst.SEG_TYPE_ACS) {
                ...
                scenRRDemand_veh[period]
                        = scenOnDemand_veh[period] - inOnDemand_veh.get(period) * minODAF;
    ...
    private void calDemand_Mainline(int scen, int atdm, int period) {
        ...
        if (inUpSeg == null) {
            ...
            //adjust mainline demand
            value = inUpSeg.scenMainlineDemand_veh[period] - inUpSeg.scenOffDemand_veh[period]
                    + scenOnDemand_veh[period];
            scenMainlineDemand_veh[period] = value;
    ...
    private void calWeaving(int period) {
        ...
        vFF_pc = CEHelper.veh_to_pc(scenMainlineDemand_veh[period], inMainlineFHV[period])/*scenMainlineDemand_pc[period]*/
                - CEHelper.veh_to_pc(scenOnDemand_veh[period], inMainlineFHV[period])/*scenOnDemand_pc[period]*/
                - vFR_pc;
    ...
    private float funcBasicMainlineCapacity(int scen, int atdm, int period) {
        float result = CEHelper.pc_to_veh(funcIdealCapacity(scenMainlineFFS[period], period)
                * scenMainlineNumLanes[period],
    ...
    private float funcWeaveMainlineCapacity(int scen, int atdm, int period) {
        //HCM Page 12-17
        float cIWL, cIW, cW;
        float cIFL = funcIdealCapacity(scenMainlineFFS[period], period);
     ...
     */
    private float funcIdealCapacity(float freeFlowSpeed, int period) {
        //pc/h/ln
...
//                        scenAllDensity_pc[period] = CEHelper.veh_to_pc(scenAllDensity_veh[period], inMainlineFHV[period]);
    ...
    void calExtendedResults() {
        for (int period = 0; period < inNumPeriod; period++) {
            ...
            scenOnDelay[period]
                    = inType == CEConst.SEG_TYPE_ONR || inType == CEConst.SEG_TYPE_W || inType == CEConst.SEG_TYPE_ACS
                            ? testOnRampDelay[period] : 0;
            scenSysDelay[period]
                    = scenMainlineDelay/*[period]*/ + scenOnDelay[period];
            ...
            scenVHT[period]
                    = scenActualTime/*[period]*/ * scenMainlineVolume_veh[period] / 240;
            scenVHD[period]
                    = scenMainlineDelay/*[period]*/ * scenMainlineVolume_veh[period] / 240
